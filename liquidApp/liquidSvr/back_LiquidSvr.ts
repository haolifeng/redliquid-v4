        ...
        if(txMap.size > 0){
            for(let [userAddr, accountData] of txMap){
                try{
                    ...
                    logger.debug('==================================================   head ')
                    logger.debug('userAddress: ', userAddr, ' health: ', accountData.health);

                    logger.debug('============================ begin liquid ======================    ');
                    ...
                    logger.info('-1 -- the select Borrow Market is -  mostIndex :', mostIndex);
                    ...
                    logger.info(' 0 -- from BorrowMarket: bnMostActualReapyAmount : ', bnMostActualRepayAmount.toString());
                    ...
                    logger.info('1 -- from supplyMarket: bnMostActualRepayAmountTemp : ', bnMostActualRepayAmountTemp.toString());
                    //bnMostActualRepayAmountTemp = bnMostActualRepayAmountTemp.mul(BigNumber.from('3')).div(BigNumber.from('4'));

                    let myBalance = await computeBorrowAmountFromCollateWan(liquidMarketData.cTokenAddress);
                    if(!myBalance){
